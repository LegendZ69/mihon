        ...
        if (s.pages.any { (it.page as? ImagePage.ImageSingle)?.hasOverlay == true }) {
            overlayPass(encoder, texture) { pass ->
                s.pages.forEach { vp ->
                    ...
                    val pageScale = dstW / page.width
                    val imageScale = pageScale * s.scale
                    page.forEachOverlay { image, offsetX, overlay ->
                        val targetX = anchorX + s.scale * pageScale * (offsetX + image.x)
                        val targetY = anchorY + s.scale * (vp.docTop + 0.5f * vp.pageHeight + pageScale * image.y)
