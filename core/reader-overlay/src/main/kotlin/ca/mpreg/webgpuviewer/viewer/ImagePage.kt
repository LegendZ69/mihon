...
import ca.mpreg.webgpuviewer.renderer.beginRenderPassWithColorView
import ca.mpreg.webgpuviewer.renderer.endAndClose
import ca.mpreg.webgpuviewer.renderer.Image
import ca.mpreg.webgpuviewer.renderer.ImageOverlay
import ca.mpreg.webgpuviewer.renderer.overlayPass
...
open class ImagePage {
    ...
    open class Render(override val width: Int, override val height: Int) : ImagePage() {
        ...
        private fun openPassAndRender(
            ...
            val openedPass = encoder.beginRenderPassWithColorView(dst) { view ->
    ...
    open class ImageSingle(val image: Image?) : ImagePage() {
        ...
        @Volatile private var overlay: ImageOverlay? = null

        /** Ownership transfers to this page; replacing/evicting the page releases old textures. */
        suspend fun replaceOverlay(value: ImageOverlay?) = WebGpuRenderer.onDispatcher {
            if (destroyed) {
                value?.release()
        ...
        internal open val hasOverlay: Boolean get() = overlay != null

        internal open fun forEachOverlay(action: (Image, Float, Float, ImageOverlay) -> Unit) {
            val value = overlay ?: return
            currentImage?.let { action(it, 0f, 1f, value) }
        }
        ...
        private fun drawOverlays(
            ...
            if (destroyed || !hasOverlay) return
            overlayPass(encoder, dst) { pass ->
                forEachOverlay { img, offsetX, imageScale, value ->
                    val placeX = (this.x + x + offsetX / dst.width + WebGpuRenderer.offsetX) /
                            imageScale - WebGpuRenderer.offsetX
                    val placeY = (this.y + y + WebGpuRenderer.offsetY) /
                            imageScale - WebGpuRenderer.offsetY
                    value.draw(pass, dst, img, placeX, placeY, this.scale * scale * imageScale)
                }
            }
        }
        ...
        override fun renderWith(
            ...
            renderContent(encoder, x, y, scale, dst)
            drawOverlays(encoder, dst, x, y, scale)
        }
        ...
        private fun beginLivePass(
            encoder: GPUCommandEncoder, dst: GPUTexture, tiles: TileRenderer
        ) = encoder.beginRenderPassWithColorView(dst) { view ->
            GPURenderPassDescriptor(
                colorAttachments = arrayOf(
                    GPURenderPassColorAttachment(
                        view = view,
        ...
        private fun beginCachePass(encoder: GPUCommandEncoder, tex: GPUTexture) =
            encoder.beginRenderPassWithColorView(tex) { view ->
                GPURenderPassDescriptor(
                    colorAttachments = arrayOf(
                        GPURenderPassColorAttachment(
                            view = view,
        ...
        override fun drawLive(
            ...
            val covered = drawLiveContent(encoder, dst, tiles)
            drawOverlays(encoder, dst)
            return covered
        }

        private fun drawLiveContent(
            encoder: GPUCommandEncoder, dst: GPUTexture, tiles: TileRenderer
        ): Boolean {
            if (isAnimated) {
            ...
            if (!highQuality) {
                renderContent(encoder, 0f, 0f, 1f, dst)
                return false
            }
        ...
        override fun renderCacheSeed(
            ...
            renderCacheContent(encoder, tex, tiles)
            drawOverlays(encoder, tex)
        }

        private fun renderCacheContent(
            encoder: GPUCommandEncoder, tex: GPUTexture, tiles: TileRenderer
        ) {
            if (isAnimated) {
            ...
            if (!highQuality) {
                renderContent(encoder, 0f, 0f, 1f, tex)
                return
            }
        ...
        override fun renderIntoCache(
            ...
            if (!identityMatches || hasOverlay) {
                renderCacheSeed(encoder, tex, tiles)
                return
            }
            val pass = encoder.beginRenderPassWithColorView(tex) { view ->
                GPURenderPassDescriptor(
                    colorAttachments = arrayOf(
                        GPURenderPassColorAttachment(
                            view = view,
        ...
        override fun cleanup() {
            ...
            overlay = null
            if (oldOverlay != null) cleanupScope.launch {
                WebGpuRenderer.onDispatcher { oldOverlay.release() }
            }
    ...
    class ImageSpread(val left: ImagePage?, val right: ImagePage?) : ImageSingle(null) {
        ...
        override val hasOverlay: Boolean
            get() = leftSingle?.hasOverlay == true || rightSingle?.hasOverlay == true

        override fun forEachOverlay(action: (Image, Float, Float, ImageOverlay) -> Unit) {
            leftSingle?.forEachOverlay { image, offset, imageScale, overlay ->
                action(image, (offset - 0.5f * image.width) * leftScale, imageScale * leftScale, overlay)
            }
            rightSingle?.forEachOverlay { image, offset, imageScale, overlay ->
                action(image, (offset + 0.5f * image.width) * rightScale, imageScale * rightScale, overlay)
            }
        }
