...
class PaddleOCR private constructor(
    ...
    suspend fun recognize(bitmap: Bitmap): OCRRunResult = recognize(bitmap, null)

    suspend fun recognize(bitmap: Bitmap, observer: OCRStageObserver?): OCRRunResult {
        if (bitmap.width == 0 || bitmap.height == 0) {
            throw OCRError.InvalidImage()
        }
        return recognizeResult { engine.run(bitmap, observer) }
    }
    ...
    private suspend fun recognizeResult(runEngine: suspend () -> OCREngineResult): OCRRunResult {
        return withContext(Dispatchers.IO) {
