...
object CTCDecoder {
    ...
    fun decode(output: FloatArray, shape: LongArray, characterList: List<String>): List<Pair<String, Float>> {
        require(shape.size == 3 && shape.all { it in 1..Int.MAX_VALUE.toLong() }) { "Invalid recognition output shape" }
        require(
            shape.fold(1L) { size, dimension ->
                Math.multiplyExact(size, dimension)
            } == output.size.toLong(),
        ) { "Recognition output shape does not match data" }
        require(shape[2] == characterList.size.toLong() + 1) { "Recognition dictionary does not match model classes" }
